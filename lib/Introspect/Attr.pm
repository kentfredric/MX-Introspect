...
sub _unknown_definition_context {
    warn "\e[31m classmap\e[0mCan't get a definition context for "
      . $_[0]->_jobname . " "
      . $_[0]->metaattribute;
...
sub inflate_accessor {
    ...
    return Introspect::Attr::Accessor->for_accessor( $_[0]->classname,
        $_[0]->attrname, $_[1] );
}

sub accessors {
    return map { $_[0]->inflate_accessor( $_->name ) }
    $_[0]->_cc_get_attribute_methods( $_[0]->metaattribute );
    return ();
}
