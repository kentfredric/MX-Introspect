...
has constructorarg => ( is => ro =>, required => 1 );
...
sub _jobname {
  $_[0]->classname . '/' . $_[0]->attrname . '/' . $_[0]->constructorarg;
}

sub for_constructorarg {
  my ( $self, $class, $attr, $constructorarg ) = @_;
  return $self->_cache_or_construct(
    $registry,
    [ $class, $attr, $constructorarg ],
    sub {
      $self->new(
        classname      => $class,
        attrname       => $attr,
...
sub _build_is_public { return $_[0]->constructorarg !~ /^_/; }
...
sub long_explanation {
  ...
  return $dc->{package} . '/attribute = ' . $_[0]->attrname . ' => ' . $_[0]->terse_explanation;
}
