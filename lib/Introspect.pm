...
has all_requires =>
  ( is => ro =>, lazy => 1, builder => '_scrape_all_requires' );

has 'methods_public' =>
  ( is => ro =>, lazy => 1, builder => '_scrape_methods_public' );
has 'methods_private' =>
  ( is => ro =>, lazy => 1, builder => '_scrape_methods_private' );
has 'requires_public' =>
  ( is => ro =>, lazy => 1, builder => '_scrape_requires_public' );
has 'requires_private' =>
  ( is => ro =>, lazy => 1, builder => '_scrape_requires_private' );
...
sub _scrape_composes {
    ...
    if ( $_[0]->metaclass->can('calculate_all_roles_with_inheritance') ) {
        $classes->{ $_->name } = 1
          for $_[0]->metaclass->calculate_all_roles_with_inheritance();
...
sub _scrape_all_requires {
    if ( $_[0]->metaclass->can('get_required_method_list') ) {
        return [ map { $_->name }
              $_[0]->metaclass->get_required_method_list() ];
    }
    return [];
}
